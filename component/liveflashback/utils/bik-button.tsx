...
const cName = (variant: string) => {
  switch (variant) {
    ...
    case "gray":
      return "bg-[#ADADAD]/40 text-primary-700 disabled:bg-slate-300 disabled:grayscale disabled:pointer-events-none disabled:text-[#4370FF]";
    default:
      return "bg-primary text-white disabled:bg-slate-300 disabled:grayscale disabled:pointer-events-none disabled:text-[#4370FF]";
  }
};
...
const commonButtonStyles = "px-3 py-1 rounded-8 disabled:bg-slate-300 disabled:grayscale disabled:pointer-events-none";
const disabledStyles = "disabled:bg-slate-500 disabled:grayscale disabled:pointer-events-none";
...
export const Button = ({ children, title, type = "button", variant = "primary", onClick, className, disabled, loading, toolTip }: ButtonProps2) => {
  return (
    <button
      title={toolTip}
      type={type}
...
export const ButtonGray: React.FC<ButtonProps> = ({ title, type = "button", onClick, className = "", disabled = false, loading = false }) => {
  return (
    <button type={type} onClick={onClick} disabled={disabled} className={cn(`bg-primary-100 text-primary-700 relative ${commonButtonStyles}`, className)}>
      {title}
      {loading ? (
        <div className="text-primary absolute top-0 left-0 size-full">
          <ButtonLoading />
...
export const ButtonSecondary: React.FC<ButtonProps> = ({ title, type = "button", onClick, className = "", disabled = false, loading = false }) => {
  return (
    <button
      type={type}
      onClick={onClick}
      disabled={disabled}
      className={cn(`bg-secondary relative text-white ${commonButtonStyles} ${disabledStyles}`, className)}
    >
      {title}
      {loading ? (
        <div className="text-primary absolute top-0 left-0 size-full">
...
export const ButtonGr: React.FC<ButtonProps> = ({ title, type = "button", onClick, className = "", disabled = false, loading = false }) => {
  return (
    <button type={type} onClick={onClick} disabled={disabled} className={cn("relative bg-[#17dc69] text-white", commonButtonStyles, className)}>
      {title}
      {loading ? (
        <div className="text-primary absolute top-0 left-0 size-full">
          <ButtonLoading />
...
export const ButtonPinkOutline: React.FC<ButtonProps> = ({ title, type = "button", onClick, className = "", disabled = false, loading = false }) => {
  return (
    <button
      type={type}
      onClick={onClick}
      disabled={disabled}
      className={cn("hover:bg-pink text-pink relative bg-[rgba(245,0,87,0.1)] transition-colors hover:text-white", commonButtonStyles, className)}
    >
      {title}
      {loading ? (
        <div className="text-primary absolute top-0 left-0 size-full">
...
export const ButtonSecondaryLine: React.FC<ButtonProps> = ({ title, type = "button", onClick, className = "", disabled = false, loading = false }) => {
  return (
    <button
      type={type}
      onClick={onClick}
      disabled={disabled}
      className={cn("group bg-secondary-100 hover:bg-secondary relative transition-colors", commonButtonStyles, className)}
    >
...
export const ButtonReload: React.FC<Partial<ButtonProps>> = ({ onClick, className = "" }) => {
  return (
